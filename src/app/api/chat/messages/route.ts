...
import { ConversationUpdateData, MessageStatus, MessageType, SendMessageApiResponse, SendMessageRequest } from '../../../../types';
...
export async function POST(request: Request): Promise<NextResponse<SendMessageApiResponse>> {
  try {
    ...
    const { conversationId, content, type = MessageType.Text, attachments } = body;
    ...
    const sender: IUser | null = await User.findById(createdMessage.senderId)
